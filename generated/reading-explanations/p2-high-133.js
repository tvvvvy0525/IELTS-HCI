(function registerReadingExplanationData(global) {
  'use strict';
  if (!global.__READING_EXPLANATION_DATA__ || typeof global.__READING_EXPLANATION_DATA__.register !== "function") {
    throw new Error("reading_explanation_registry_missing");
  }
  global.__READING_EXPLANATION_DATA__.register("p2-high-133", {
  "schemaVersion": "ReadingExplanationV1",
  "examId": "p2-high-133",
  "meta": {
    "examId": "p2-high-133",
    "title": "Playing soccer 街头足球",
    "category": "P2",
    "sourceDoc": "P2 33-59之_副本.md",
    "noteType": "总结",
    "matchedTitle": "Playing soccer 街头足球【高】"
  },
  "passageNotes": [
    {
      "label": "Paragraph A",
      "text": "街头足球是足球的非正式形式，常见于城市街道，普及原因众多：无需大场地，无正式比赛所需的球门柱、角旗、标线及裁判；无需 11 人制球队和固定位置；新手无需重复技术战术训练，通过比赛中的失误自然学习（无成人或教练指导），在玩乐中轻松掌握足球直觉。"
    },
    {
      "label": "Paragraph B",
      "text": "如今许多城市已难以开展街头足球：交通拥堵占据原游戏场地，公园成为青少年聚集地，多地需许可才能使用公共空间，且年轻人日程繁忙，自发游戏几乎消失。"
    },
    {
      "label": "Paragraph C",
      "text": "除环境障碍外，社会学角度也能解释街头足球 popularity 下降：美国作家富兰克林・福尔（Franklin Foer）在《足球如何解释世界》中指出，20 世纪 60 年代育儿理念既追求自由也注重保护 —— 如同消费者运动推动汽车配备安全带和安全气囊，青少年足球运动试图通过规则保护儿童身心，例如联赛给所有球员发 “参与奖”，部分家长担忧头球伤脑（尽管无医学证据），催生防护装备产业，部分联赛甚至禁止头球。"
    },
    {
      "label": "Paragraph D",
      "text": "越来越多人认为街头足球的教育方式不正规，主张儿童需专家指导 —— 青少年足球从 4 岁开始培训，家长担心落后，这种指导排斥街头足球的试错模式。"
    },
    {
      "label": "Paragraph E",
      "text": "街头足球中，优秀球员为新手分配特定角色，强调团队利益；而青少年足球主张 “每个孩子需学习所有位置”，两者理念相反。街头足球的角色随比赛变化，核心是 “发挥个人当下最大优势”。"
    },
    {
      "label": "Paragraph F",
      "text": "街头足球中，儿童需学会耐心等待、接受无决策权的现实，责任职位需通过队内竞争获得，年轻球员需等待机会；而青少年足球强调 “过度民主”，保证每个孩子有表现机会（如轮流当队长、踢核心位置），使孩子误以为无需努力和耐心。"
    },
    {
      "label": "Paragraph G",
      "text": "青少年足球的原则是 “人人获胜，无人失败”，确保孩子积极离场，但这削弱了努力的动力；街头足球有明确胜负，球员早早学会应对失败，培养心理韧性。此外，街头足球无正式记录，胜负转瞬即忘；青少年足球则记录球员位置和赛季成绩。"
    }
  ],
  "questionExplanations": [
    {
      "sectionTitle": "1. 段落匹配（Questions 14–19：Which section contains the following information?）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 14,
          "text": "（1）题目 14：a contrast between the ways young players gain experience of playing different positions（对比年轻球员体验不同位置的方式）\n答案：E\n解析：定位 Paragraph E 中 “One of the basic ideas of street soccer is that young players are assigned a particular role... Such an assignment runs counter to the idea of youth soccer that every child needs to learn every position”，对比街头足球 “分配特定角色” 与青少年足球 “学习所有位置”，与题干对应，因此答案为 E。",
          "questionId": "q1"
        },
        {
          "questionNumber": 15,
          "text": "（2）题目 15：examples outside sport of greater emphasis on individual safety（体育外重视个人安全的例子）\n答案：C\n解析：定位 Paragraph C 中 “Like the 1960s consumer movement which brought seat belts and airbags to cars, the (youth) soccer movement felt like it could create a set of … regulations that would protect both the child’s body and mind from damage”，以 “汽车安全带、安全气囊” 为例，与题干对应，因此答案为 C。",
          "questionId": "q2"
        },
        {
          "questionNumber": 16,
          "text": "（3）题目 16：a description of methods of selection for leadership on soccer teams（描述足球队 leadership 选拔方式）\n答案：F\n解析：定位 Paragraph F 中 “Positions of responsibility are earned through competition within the team. Younger players in street soccer must wait to attain those positions... youth soccer players come to believe that hard work and patience aren’t really necessary”，描述街头足球 “竞争 + 等待” 与青少年足球 “无差别分配” 的选拔差异，与题干对应，因此答案为 F。",
          "questionId": "q3"
        },
        {
          "questionNumber": 17,
          "text": "（4）题目 17：details of urban changes that discourage street soccer（阻碍街头足球的城市变化细节）\n答案：B\n解析：定位 Paragraph B 中 “Congested traffic now dominates... Parks and open fields are used as hangouts... requirement... for official permits... managed schedules”，列举交通、场地用途、许可制度等城市变化，与题干对应，因此答案为 B。",
          "questionId": "q4"
        },
        {
          "questionNumber": 18,
          "text": "（5）题目 18：a mention of the lesson that failure teaches street soccer players（提及失败给街头足球球员带来的教训）\n答案：G\n解析：定位 Paragraph G 中 “Losing a game is a common experience and players learn early on how to handle this. As a result, unlike most youth soccer players, they acquire resilience”，明确提到失败教会球员 “应对能力和韧性”，与题干对应，因此答案为 G。",
          "questionId": "q5"
        },
        {
          "questionNumber": 19,
          "text": "（6）题目 19：an explanation of why youth soccer emphasises the need for coaches（解释青少年足球强调教练必要性的原因）\n答案：D\n解析：定位 Paragraph D 中 “They argue that children need to be taught by experts... This need to get ahead brings with it a fear of falling behind that only expert instruction can prevent”，解释 “担心落后，需专家指导”，与题干对应，因此答案为 D。",
          "questionId": "q6"
        }
      ],
      "questionRange": {
        "start": 14,
        "end": 19
      },
      "text": "答案：E\n解析：定位 Paragraph E 中 “One of the basic ideas of street soccer is that young players are assigned a particular role... Such an assignment runs counter to the idea of youth soccer that every child needs to learn every position”，对比街头足球 “分配特定角色” 与青少年足球 “学习所有位置”，与题干对应，因此答案为 E。\n答案：C\n解析：定位 Paragraph C 中 “Like the 1960s consumer movement which brought seat belts and airbags to cars, the (youth) soccer movement felt like it could create a set of … regulations that would protect both the child’s body and mind from damage”，以 “汽车安全带、安全气囊” 为例，与题干对应，因此答案为 C。\n答案：F\n解析：定位 Paragraph F 中 “Positions of responsibility are earned through competition within the team. Younger players in street soccer must wait to attain those positions... youth soccer players come to believe that hard work and patience aren’t really necessary”，描述街头足球 “竞争 + 等待” 与青少年足球 “无差别分配” 的选拔差异，与题干对应，因此答案为 F。\n答案：B\n解析：定位 Paragraph B 中 “Congested traffic now dominates... Parks and open fields are used as hangouts... requirement... for official permits... managed schedules”，列举交通、场地用途、许可制度等城市变化，与题干对应，因此答案为 B。\n答案：G\n解析：定位 Paragraph G 中 “Losing a game is a common experience and players learn early on how to handle this. As a result, unlike most youth soccer players, they acquire resilience”，明确提到失败教会球员 “应对能力和韧性”，与题干对应，因此答案为 G。\n答案：D\n解析：定位 Paragraph D 中 “They argue that children need to be taught by experts... This need to get ahead brings with it a fear of falling behind that only expert instruction can prevent”，解释 “担心落后，需专家指导”，与题干对应，因此答案为 D。"
    },
    {
      "sectionTitle": "2. 多选题（Questions 20–21：Choose TWO letters）",
      "mode": "per_question",
      "items": [],
      "questionRange": {
        "start": 20,
        "end": 21
      },
      "text": "题目：Which TWO of these reasons are mentioned by the writer of the text?（作者提及的街头足球受欢迎的两个原因）\n答案：C、E\n选项 C：You do not need elaborate facilities to play street soccer.（无需复杂设施即可玩街头足球）\n解析：定位 Paragraph A 中 “no large space is needed, and goal posts, corner markers, and marked lines... are typically absent”，明确提到无需复杂设施，C 正确。\n- 选项 E：Street soccer teams can have varying numbers of players.（街头足球队人数灵活）\n解析：定位 Paragraph A 中 “does not necessarily require standard 11-a-side teams”，明确提到无需固定 11 人制，人数灵活，E 正确。\n- 排除 A：文中无 “著名球员从街头足球起步” 的表述；排除 B：文中未提及 “街头足球适合低龄儿童”；排除 D：文中无 “新手不被批评” 的表述。"
    },
    {
      "sectionTitle": "3. 多选题（Questions 22–23：Choose TWO letters）",
      "mode": "per_question",
      "items": [],
      "questionRange": {
        "start": 22,
        "end": 23
      },
      "text": "题目：Which TWO of these results are mentioned by Franklin Foer?（富兰克林・福尔提及的 20 世纪 60 年代育儿风格的两个结果）\n答案：B、D\n选项 B：Some youth soccer leagues adopted more restrictive rules of play.（部分青少年足球联赛采用更严格的比赛规则）\n解析：定位 Paragraph C 中 “some youth leagues prohibited heading the ball altogether”，明确提到禁止头球等严格规则，B 正确。\n- 选项 D：Youth soccer players were sometimes rewarded for simply playing in games.（青少年足球球员有时仅因参赛获奖励）\n解析：定位 Paragraph C 中 “Soccer leagues... handed out ‘participation’ prizes to every player, no matter how few games his (or her) team won”，明确提到 “参与奖”，D 正确。\n- 排除 A：文中无 “青少年足球成本上升” 的表述；排除 C：文中未提及 “参与人数减少”；排除 E：文中无 “厂商针对家长广告” 的表述。"
    },
    {
      "sectionTitle": "4. 摘要填空（Questions 24–26：ONE WORD ONLY）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 24,
          "text": "（1）题目 24：For youth soccer players, a key \\___\\___ is that they should always come away from the game with a positive attitude.\n题目翻译：对青少年足球球员而言，核心_\\___\\__是他们离场时应始终保持积极态度。\n答案：principle（原则）\n解析：定位 Paragraph G 中 “‘Everyone’s a winner; no one’s a loser’ is a guiding principle of youth soccer. This ensures each individual goes away positive about themselves”，明确提到 “原则”，因此答案为 “principle”。",
          "questionId": "q11"
        },
        {
          "questionNumber": 25,
          "text": "（2）题目 25：In this respect, regardless of the effort the players make, they get some kind of \\___\\___ at the end of a game.\n题目翻译：因此，无论球员付出多少努力，赛后都会获得某种_\\___\\__。\n答案：reward（奖励）\n解析：定位 Paragraph G 中 “giving less than your best receives the same reward as giving your best”，明确提到 “奖励”，因此答案为 “reward”。",
          "questionId": "q12"
        },
        {
          "questionNumber": 26,
          "text": "（3）题目 26：In fact, no-one ever keeps a \\___\\___ of the results of games.\n题目翻译：事实上，无人记录比赛结果的_\\___\\__。\n答案：record（记录）\n解析：定位 Paragraph G 中 “in street soccer a formal record is not kept”，明确提到 “记录”，因此答案为 “record”。",
          "questionId": "q13"
        }
      ],
      "questionRange": {
        "start": 24,
        "end": 26
      },
      "text": "题目翻译：对青少年足球球员而言，核心_\\___\\__是他们离场时应始终保持积极态度。\n答案：principle（原则）\n解析：定位 Paragraph G 中 “‘Everyone’s a winner; no one’s a loser’ is a guiding principle of youth soccer. This ensures each individual goes away positive about themselves”，明确提到 “原则”，因此答案为 “principle”。\n题目翻译：因此，无论球员付出多少努力，赛后都会获得某种_\\___\\__。\n答案：reward（奖励）\n解析：定位 Paragraph G 中 “giving less than your best receives the same reward as giving your best”，明确提到 “奖励”，因此答案为 “reward”。\n题目翻译：事实上，无人记录比赛结果的_\\___\\__。\n答案：record（记录）\n解析：定位 Paragraph G 中 “in street soccer a formal record is not kept”，明确提到 “记录”，因此答案为 “record”。"
    }
  ]
}
  );
})(typeof window !== "undefined" ? window : globalThis);
