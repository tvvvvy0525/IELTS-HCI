(function registerReadingExplanationData(global) {
  'use strict';
  if (!global.__READING_EXPLANATION_DATA__ || typeof global.__READING_EXPLANATION_DATA__.register !== "function") {
    throw new Error("reading_explanation_registry_missing");
  }
  global.__READING_EXPLANATION_DATA__.register("p1-high-227", {
  "schemaVersion": "ReadingExplanationV1",
  "examId": "p1-high-227",
  "meta": {
    "examId": "p1-high-227",
    "title": "The Whale Goes to Court 鲸鱼油",
    "category": "P1",
    "sourceDoc": "227. P1 - The Whale Goes to Court 鲸鱼油.pdf",
    "noteType": "总结",
    "matchedTitle": "The Whale Goes to Court 鲸鱼油"
  },
  "passageNotes": [
    {
      "label": "Paragraph A",
      "text": "1818 年，一头鲸鱼成为纽约市一场备受争议的法庭案件的主角。该案涉及一项古老的法律，要求卖鱼油的人必须付费，以便让城市官员检查并认证他们的油桶。然而，一位名叫塞缪尔・贾德的油商拒绝为三桶油支付检查费，他声称无需检查，因为那是鲸油，而鲸鱼不是鱼。州政府则持相反意见，因此法院确定了一个日期，要裁决的不是一个法律问题，而是一个更根本的问题的答案：鲸鱼是鱼吗？"
    },
    {
      "label": "Paragraph B",
      "text": "尽管这个问题在今天看来似乎很简单，但在 19 世纪初，答案远非显而易见。事实上，这场审判在纽约引发的公众辩论轰动一时。争论的焦点几乎触及了大多数人眼中的自然秩序。根据普遍接受的分类体系，如果一种动物不是兽类或鸟类，那它就是鱼，无论它是否呼吸空气或哺育幼崽。从有记忆以来，所有非人类生物都被分为鸟类、兽类和鱼类三大类。对普通人来说，答案似乎显而易见：鲸鱼在海里游泳，因此它们就是鱼。"
    },
    {
      "label": "Paragraph C",
      "text": "与这一传统框架相对立的是新的林奈分类系统，该系统试图将更科学的价值观引入生物分类。林奈系统在 1818 年仅有不到半个世纪的历史，却颇具争议地将鲸鱼归类为哺乳动物，因为它们具有两个哺乳动物特征：它们是温血动物且呼吸空气。"
    },
    {
      "label": "Paragraph D",
      "text": "审判于 1818 年 12 月开始。贾德的辩护律师选择了纽约最著名的人物之一、国会议员塞缪尔・米切尔作为他们的明星证人。米切尔被称为“活百科全书”，被传唤来陈述案件的生物学依据。然而，检方律师威廉・桑普森将审判变成了一场科学知识与常识之间的较量，他请直言不讳的捕鲸者在陪审团面前陈述他们的观点。"
    },
    {
      "label": "Paragraph E",
      "text": "桑普森案件的核心在于，如果贾德被判无罪，对人类会产生什么影响。桑普森告诉陪审团成员，如果他们接受米切尔关于鲸鱼的证词，他们就必须在自然秩序中接受自己物种的较低地位。因此，米切尔在法庭上的日子并不顺利，部分原因是他不得不承认同行们在如何分类生物方面存在严重分歧。桑普森随后将矛头指向米切尔本人及其所代表的群体：来自科学界、脱离现实的特权贵族。能言善辩的桑普森随后声称米切尔的信仰源于欧洲 —— 他正确地判断出这一点会激怒一贯独立自主的纽约市民。桑普森声称，这种革命性的新思维是诚实工作的纽约人所不需要的。这不再是一个关于油桶的案件，而是关于科学知识在美国的适当位置。"
    },
    {
      "label": "Paragraph F",
      "text": "最终，为塞缪尔・贾德赢得案件的并非米切尔的科学论点，而是来自完全不同领域的证词。在纽约的市场中，商人们心照不宣地明白，鲸油和鱼油并不相同：鲸油可以用作灯的燃料，因为它燃烧时不会冒烟；而鱼油则是肮脏、不纯的物质，主要用于皮革鞣制。最终，正是这些证词让贾德获得了胜利。"
    },
    {
      "label": "Paragraph G",
      "text": "鲸鱼的审判在今天仍然具有现实意义，因为舆论法庭仍然很容易被怀疑论者左右。1818 年的审判真正关乎的问题是：由谁来决定自然世界在人类世界中的位置 —— 是科学专家还是公众舆论？无论今天争论的议题是人类活动对气候的影响还是其他问题，这场辩论仍在继续。"
    }
  ],
  "questionExplanations": [
    {
      "sectionTitle": "1. 判断正误（Questions 1–7）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 1,
          "text": "（1）题目 1：An inspection fee on fish oil was introduced in New York in 1818.\n题目翻译：1818 年，纽约引入了对鱼油的检查费。\n答案：FALSE\n解析：定位 Paragraph A 中 “The case involved an old law requiring those who sold fish oil to pay a fee in order to have their barrels inspected”。文中明确指出这是一项“古老的法律”（old law），说明检查费在 1818 年之前就已存在，而非 1818 年新引入的，与题干矛盾，因此答案为 FALSE。",
          "questionId": "q1"
        },
        {
          "questionNumber": 2,
          "text": "（2）题目 2：Samuel Judd argued that the inspection fee should exclude whale oil.\n题目翻译：塞缪尔・贾德认为检查费不应适用于鲸油。\n答案：TRUE\n解析：定位 Paragraph A 中 “an oil merchant named Samuel Judd refused to pay the inspection fee on three barrels of oil, claiming that no inspection was necessary because it was whale oil, and whales were not fish”。贾德拒绝支付检查费，理由是鲸油不需要检查，因为鲸鱼不是鱼，与题干一致，因此答案为 TRUE。",
          "questionId": "q2"
        },
        {
          "questionNumber": 3,
          "text": "（3）题目 3：Judd had been in trouble with city officials before the inspection fee disagreement.\n题目翻译：贾德在检查费争议之前就曾与城市官员发生过冲突。\n答案：NOT GIVEN\n解析：文中仅提及贾德因拒绝支付检查费而与州政府产生争议，并未提及他此前是否曾与城市官员有过任何冲突或纠纷，因此答案为 NOT GIVEN。",
          "questionId": "q3"
        },
        {
          "questionNumber": 4,
          "text": "（4）题目 4：Many New Yorkers were interested in the court case at the time.\n题目翻译：当时许多纽约人对这起法庭案件很感兴趣。\n答案：TRUE\n解析：定位 Paragraph B 中 “the public debate in New York sparked off by the trial was sensational”。审判引发的公众辩论轰动一时，说明大量纽约人关注此案，与题干一致，因此答案为 TRUE。",
          "questionId": "q4"
        },
        {
          "questionNumber": 5,
          "text": "（5）题目 5：Traditionally, non-human creatures had been classified in one of three groups.\n题目翻译：传统上，非人类生物被分为三大类之一。\n答案：TRUE\n解析：定位 Paragraph B 中 “all non-human creatures had been organised according to the categories of birds, beasts and fish”。文中明确指出所有非人类生物传统上被分为鸟类、兽类和鱼类三类，与题干一致，因此答案为 TRUE。",
          "questionId": "q5"
        },
        {
          "questionNumber": 6,
          "text": "（6）题目 6：Generally speaking, ordinary people thought fish were the lowest form of life.\n题目翻译：一般来说，普通人认为鱼类是最低等的生命形式。\n答案：NOT GIVEN\n解析：文中仅提及普通人认为鲸鱼在海里游泳所以是鱼，但并未提及普通人认为鱼类是最低等的生命形式这一观点，因此答案为 NOT GIVEN。",
          "questionId": "q6"
        },
        {
          "questionNumber": 7,
          "text": "（7）题目 7：Whales were excluded from the Linnaean system in 1818.\n题目翻译：1818 年，鲸鱼被排除在林奈分类系统之外。\n答案：FALSE\n解析：定位 Paragraph C 中 “the Linnaean system controversially classified whales as mammals because they shared two mammalian characteristics”。林奈系统将鲸鱼归类为哺乳动物，而非将其排除在外，与题干矛盾，因此答案为 FALSE。",
          "questionId": "q7"
        }
      ],
      "questionRange": {
        "start": 1,
        "end": 7
      },
      "text": "题目翻译：1818 年，纽约引入了对鱼油的检查费。\n答案：FALSE\n解析：定位 Paragraph A 中 “The case involved an old law requiring those who sold fish oil to pay a fee in order to have their barrels inspected”。文中明确指出这是一项“古老的法律”（old law），说明检查费在 1818 年之前就已存在，而非 1818 年新引入的，与题干矛盾，因此答案为 FALSE。\n题目翻译：塞缪尔・贾德认为检查费不应适用于鲸油。\n答案：TRUE\n解析：定位 Paragraph A 中 “an oil merchant named Samuel Judd refused to pay the inspection fee on three barrels of oil, claiming that no inspection was necessary because it was whale oil, and whales were not fish”。贾德拒绝支付检查费，理由是鲸油不需要检查，因为鲸鱼不是鱼，与题干一致，因此答案为 TRUE。\n题目翻译：贾德在检查费争议之前就曾与城市官员发生过冲突。\n答案：NOT GIVEN\n解析：文中仅提及贾德因拒绝支付检查费而与州政府产生争议，并未提及他此前是否曾与城市官员有过任何冲突或纠纷，因此答案为 NOT GIVEN。\n题目翻译：当时许多纽约人对这起法庭案件很感兴趣。\n答案：TRUE\n解析：定位 Paragraph B 中 “the public debate in New York sparked off by the trial was sensational”。审判引发的公众辩论轰动一时，说明大量纽约人关注此案，与题干一致，因此答案为 TRUE。\n题目翻译：传统上，非人类生物被分为三大类之一。\n答案：TRUE\n解析：定位 Paragraph B 中 “all non-human creatures had been organised according to the categories of birds, beasts and fish”。文中明确指出所有非人类生物传统上被分为鸟类、兽类和鱼类三类，与题干一致，因此答案为 TRUE。\n题目翻译：一般来说，普通人认为鱼类是最低等的生命形式。\n答案：NOT GIVEN\n解析：文中仅提及普通人认为鲸鱼在海里游泳所以是鱼，但并未提及普通人认为鱼类是最低等的生命形式这一观点，因此答案为 NOT GIVEN。\n题目翻译：1818 年，鲸鱼被排除在林奈分类系统之外。\n答案：FALSE\n解析：定位 Paragraph C 中 “the Linnaean system controversially classified whales as mammals because they shared two mammalian characteristics”。林奈系统将鲸鱼归类为哺乳动物，而非将其排除在外，与题干矛盾，因此答案为 FALSE。"
    },
    {
      "sectionTitle": "2. 笔记填空（Questions 8–13）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 8,
          "text": "（1）题目 8：Samuel Mitchill worked as a congressman and a ______.\n题目翻译：塞缪尔・米切尔担任国会议员和 ______。\n答案：scientist\n解析：定位 Paragraph D 中 “Referred to as a 'living encyclopaedia', Mitchill was called upon to present the biology of the case”。米切尔被称为“活百科全书”，被要求陈述案件的生物学依据，说明他同时具备科学家身份。且 Paragraph A 中提及他作为“congressman”（国会议员），结合上下文，他同时是科学家（scientist），因此答案为 scientist。",
          "questionId": "q8"
        },
        {
          "questionNumber": 9,
          "text": "（2）题目 9：William Sampson called ______ as witnesses in order to appeal to the common sense of the jury.\n题目翻译：威廉・桑普森传唤 ______ 作为证人，以诉诸陪审团的常识。\n答案：whalers\n解析：定位 Paragraph D 中 “Sampson turned the trial into a contest between scientific learning and common sense, by asking plain-spoken whalers to make their case before the jury”。桑普森请直言不讳的捕鲸者（whalers）在陪审团面前陈述，以诉诸常识，因此答案为 whalers。",
          "questionId": "q9"
        },
        {
          "questionNumber": 10,
          "text": "（3）题目 10：New Yorkers disliked Mitchill because his ideas came from ______.\n题目翻译：纽约人不喜欢米切尔，因为他的观点来自 ______。\n答案：Europe\n解析：定位 Paragraph E 中 “Sampson then claimed Mitchill's beliefs had their origin in Europe – something he rightly judged would infuriate the citizens of always-independent New York”。桑普森声称米切尔的观点源于欧洲，这激怒了纽约市民，因此答案为 Europe。",
          "questionId": "q10"
        },
        {
          "questionNumber": 11,
          "text": "（4）题目 11：In the end it was statements from local ______, not Mitchill's testimony which helped Judd win.\n题目翻译：最终帮助贾德获胜的是当地 ______ 的陈述，而非米切尔的证词。\n答案：merchants\n解析：定位 Paragraph F 中 “In New York's markets, the merchants implicitly understood that whale oil and fish oil were not the same... In the end it was this testimony that led to Judd's victory”。纽约市场中的商人（merchants）提供了关键证词，帮助贾德获胜，因此答案为 merchants。",
          "questionId": "q11"
        },
        {
          "questionNumber": 12,
          "text": "（5）题目 12：Whale oil made a good ______ because it was clean.\n题目翻译：鲸油是一种很好的 ______，因为它很干净。\n答案：fuel\n解析：定位 Paragraph F 中 “whale oil could be used as a fuel for lamps because it could be burned without giving off smoke”。鲸油可用作灯的燃料（fuel），因为燃烧时不冒烟，因此答案为 fuel。",
          "questionId": "q12"
        },
        {
          "questionNumber": 13,
          "text": "（6）题目 13：Judd's case is relevant today, e.g. in the debate about Earth's ______.\n题目翻译：贾德的案件在今天仍有现实意义，例如在关于地球 ______ 的辩论中。\n答案：climate\n解析：定位 Paragraph G 中 “Whether the issue today is the effects of human activity on the climate or some other matter, the debate continues”。文中明确提及人类活动对气候（climate）的影响，因此答案为 climate。",
          "questionId": "q13"
        }
      ],
      "questionRange": {
        "start": 8,
        "end": 13
      },
      "text": "题目翻译：塞缪尔・米切尔担任国会议员和 ______。\n答案：scientist\n解析：定位 Paragraph D 中 “Referred to as a 'living encyclopaedia', Mitchill was called upon to present the biology of the case”。米切尔被称为“活百科全书”，被要求陈述案件的生物学依据，说明他同时具备科学家身份。且 Paragraph A 中提及他作为“congressman”（国会议员），结合上下文，他同时是科学家（scientist），因此答案为 scientist。\n题目翻译：威廉・桑普森传唤 ______ 作为证人，以诉诸陪审团的常识。\n答案：whalers\n解析：定位 Paragraph D 中 “Sampson turned the trial into a contest between scientific learning and common sense, by asking plain-spoken whalers to make their case before the jury”。桑普森请直言不讳的捕鲸者（whalers）在陪审团面前陈述，以诉诸常识，因此答案为 whalers。\n题目翻译：纽约人不喜欢米切尔，因为他的观点来自 ______。\n答案：Europe\n解析：定位 Paragraph E 中 “Sampson then claimed Mitchill's beliefs had their origin in Europe – something he rightly judged would infuriate the citizens of always-independent New York”。桑普森声称米切尔的观点源于欧洲，这激怒了纽约市民，因此答案为 Europe。\n题目翻译：最终帮助贾德获胜的是当地 ______ 的陈述，而非米切尔的证词。\n答案：merchants\n解析：定位 Paragraph F 中 “In New York's markets, the merchants implicitly understood that whale oil and fish oil were not the same... In the end it was this testimony that led to Judd's victory”。纽约市场中的商人（merchants）提供了关键证词，帮助贾德获胜，因此答案为 merchants。\n题目翻译：鲸油是一种很好的 ______，因为它很干净。\n答案：fuel\n解析：定位 Paragraph F 中 “whale oil could be used as a fuel for lamps because it could be burned without giving off smoke”。鲸油可用作灯的燃料（fuel），因为燃烧时不冒烟，因此答案为 fuel。\n题目翻译：贾德的案件在今天仍有现实意义，例如在关于地球 ______ 的辩论中。\n答案：climate\n解析：定位 Paragraph G 中 “Whether the issue today is the effects of human activity on the climate or some other matter, the debate continues”。文中明确提及人类活动对气候（climate）的影响，因此答案为 climate。"
    }
  ]
}
  );
})(typeof window !== "undefined" ? window : globalThis);
