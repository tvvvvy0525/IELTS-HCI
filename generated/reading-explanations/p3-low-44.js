(function registerReadingExplanationData(global) {
  'use strict';
  if (!global.__READING_EXPLANATION_DATA__ || typeof global.__READING_EXPLANATION_DATA__.register !== "function") {
    throw new Error("reading_explanation_registry_missing");
  }
  global.__READING_EXPLANATION_DATA__.register("p3-low-44", {
  "schemaVersion": "ReadingExplanationV1",
  "examId": "p3-low-44",
  "meta": {
    "examId": "p3-low-44",
    "title": "Conformity 从众心理",
    "category": "P3",
    "sourceDoc": "P3 解析+1202高频_副本.md",
    "noteType": "翻译",
    "matchedTitle": "Conformity 从众心理【中】"
  },
  "passageNotes": [
    {
      "label": "第一段",
      "text": "在你的童年时期，可能会有某种狂热影响了你学校里的所有人。它可能与某个特定的玩具有关，或者可能是一件必须拥有的衣物。它可能像一种笔那样简单，也可能像电子游戏机那样昂贵。时装设计师、玩具制造商以及任何其他涉及零售业的人都喜欢从众心理。制造一种狂热，特别是在年轻人中，所有人都会去追捧它。事实上，这是销售大量商品的理想方式。消费主义中的从众水平是惊人的。当你真正退一步，考虑我们是多么容易被说服，认为拥有某些物品是确保内心平静的唯一方式时，你会发现从众是一个多么重要的概念。"
    },
    {
      "label": "第二段",
      "text": "从众被描述为“屈服于群体压力”（Crutchfield, 1962）。然而，这意味着其他人对我们施加压力使我们从众，但这并不总是事实。阿伦森（Aronson, 1976）给出了一个更好的定义，他说这是“由于来自一个人或一群人的真实或想象的压力而导致的一个人行为或观点的改变”。这更有道理，因为我们感受到的压力往往是想象出来的。无论他们的地位如何，他所指的那个人或群体在当时对我们来说必须是重要的。"
    },
    {
      "label": "第三段",
      "text": "关于从众已经有了相当多的研究。最早的研究之一观察了人们被要求估计瓶子里豆子数量时的答案（Jenness, 1932）。如果你曾经参加过“猜数字”比赛，你可能看过之前的估计，并根据其他人猜的数字做出判断。这或多或少就是詹尼斯研究中发生的事情。首先，他要求受访者给出他们自己的估计，然后他要求他们决定一个小组估计。最后，他再次单独询问他们，发现他们坚持了小组的答案。"
    },
    {
      "label": "第四段",
      "text": "可能最著名的从众研究是由阿施（Asch, 1951）进行的，他创造了一种情境，在这个情境中，他的许多受试者给出的答案显然是不真实的，而不是去反驳与他们在一起的人。他通过让受试者与六个托儿（实验者的同事）围坐在一张桌子旁来实现这一点，受试者是倒数第二个回答的。他向他们展示了一张画有三条不同长度线条的大卡片，分别标记为A、B和C。然后他给他们一张画有一条线条的卡片，并要求他们将其长度与A、B或C中的一条线相匹配。"
    },
    {
      "label": "第五段",
      "text": "托儿在一些试验中给出了不真实的回答，受试者被置于这样一种情境中：要么报告他们亲眼所见的，要么顺从群体的规范。当结果被评估时，阿施发现，在每三次给出了错误答案的试验中，就有一名受试者给出了与托儿相同的错误答案。这导致了32%的平均从众水平。阿施在试验后采访了他的受试者，试图找出他们为什么要顺从一个如此明显错误的答案。他们中的大多数人说，他们不想在群体内部制造问题，尽管他们也表示，当他们给出错误答案时，这让他们感到焦虑。（阿施发现，如果哪怕只有一个其他人不跟随大多数人，无论其他人有多少，这都足以让受试者给出正确答案。）"
    },
    {
      "label": "第六段",
      "text": "凯尔曼（Kelman, 1953）概述了三个可以解释社会从众的过程。第一个是顺从（compliance），即受试者随大流以防止任何群体内部的敌意或不好的感觉，并维持群体和谐。然而，他们并没有改变自己的私人信念。如果我们回顾阿施的研究，我们可以看到受试者只是顺从实验情境的要求，但并没有真正内化群体的规范。他们在公开场合表示同意，但在私下里持不同意见。然而，在一个被称为内化（internalization）的过程中，受试者确实将群体的观点视为更有效的观点。他们可能通过说服自己视力不佳来做到这一点。然而，有时受试者似乎真的改变了他们的信念，因为他们想变得更像他们的英雄。如果他们真的想成为一个内群体的一部分，他们将开始认同该群体，并接受该群体的价值观和信念，即使这些与他们自己的不同。凯尔曼称此为认同（identification）。这经常发生在想要变得更像同龄群体以便被接受的青少年身上，他们似乎突然反对父母的所有价值观和信念。"
    },
    {
      "label": "第七段",
      "text": "那么为什么我们必须从众呢？有些人大部分时间都感到自信，有很高的自尊，不必随大流。但对于我们大多数人来说，我们的自信程度每天都在变化，取决于我们所处的情况，这会影响行为。"
    }
  ],
  "questionExplanations": [
    {
      "sectionTitle": "1. 判断题（Questions 27–30）",
      "mode": "per_question",
      "items": [
        {
          "questionNumber": 27,
          "text": "(1) 题目 27：Childhood crazes can centre on items of any value. （童年狂热可以集中在任何价值的物品上。）\n答案：YES\n解析：定位第1段。原文提到：\"It may have been something as simple as a type of pen \\[cheap\\] or as expensive as an electronic games console \\[expensive\\]\"（它可能像一种笔那样简单，也可能像电子游戏机那样昂贵）。无论是便宜还是昂贵，都可以成为狂热的对象。",
          "questionId": "q1"
        },
        {
          "questionNumber": 28,
          "text": "(2) 题目 28：Children are more vulnerable to crazes now than they used to be. （现在的孩子比过去更容易受到狂热的影响。）\n答案：NOT GIVEN\n解析：第1段提到\"especially in the young\"（特别是在年轻人中），说明孩子容易受影响，但并未比较“现在”与“过去”的程度差异。",
          "questionId": "q2"
        },
        {
          "questionNumber": 29,
          "text": "(3) 题目 29：Consumers make too many quick decisions in shops. （消费者在商店里做出了太多快速的决定。）\n答案：NOT GIVEN\n解析：第1段提到\"we are persuaded that having certain items...\"（我们被说服拥有某些物品……），但未提及做决定的速度是否“too quick”（太快）。",
          "questionId": "q3"
        },
        {
          "questionNumber": 30,
          "text": "(4) 题目 30：Crutchfield's definition of conformity is the most reliable. （克拉奇菲尔德对从众的定义是最可靠的。）\n答案：NO\n解析：定位第2段。原文先引用了Crutchfield的定义，随后说\"However... A better definition is given by Aronson\"（然而……阿伦森给出了一个更好的定义）。作者认为阿伦森的定义更好，因此Crutchfield的不是最可靠的。",
          "questionId": "q4"
        }
      ],
      "questionRange": {
        "start": 27,
        "end": 30
      },
      "text": "答案：YES\n解析：定位第1段。原文提到：\"It may have been something as simple as a type of pen \\[cheap\\] or as expensive as an electronic games console \\[expensive\\]\"（它可能像一种笔那样简单，也可能像电子游戏机那样昂贵）。无论是便宜还是昂贵，都可以成为狂热的对象。\n答案：NOT GIVEN\n解析：第1段提到\"especially in the young\"（特别是在年轻人中），说明孩子容易受影响，但并未比较“现在”与“过去”的程度差异。\n答案：NOT GIVEN\n解析：第1段提到\"we are persuaded that having certain items...\"（我们被说服拥有某些物品……），但未提及做决定的速度是否“too quick”（太快）。\n答案：NO\n解析：定位第2段。原文先引用了Crutchfield的定义，随后说\"However... A better definition is given by Aronson\"（然而……阿伦森给出了一个更好的定义）。作者认为阿伦森的定义更好，因此Crutchfield的不是最可靠的。"
    },
    {
      "sectionTitle": "2. 摘要填空（Questions 31–35）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 31,
          "text": "(1) 题目 31：In the Jenness study, people had to guess how many 31 \\___\\___ were in a container. （在詹尼斯的研究中，人们必须猜测容器里有多少31 \\___\\___。）\n答案：beans\n解析：定位第3段。原文：\"estimate the number of beans in a bottle\"（估计瓶子里豆子的数量）。",
          "questionId": "q5"
        },
        {
          "questionNumber": 32,
          "text": "(2) 题目 32：Jenness found that, in most cases, people opted for an estimate given by a 32 \\___\\___. （詹尼斯发现，在大多数情况下，人们选择了由32 \\___\\___给出的估计。）\n答案：group\n解析：定位第3段末尾。原文：\"discovered that they had stayed with the group answer\"（发现他们坚持了小组的答案）。",
          "questionId": "q6"
        },
        {
          "questionNumber": 33,
          "text": "(3) 题目 33：Asch asked his subjects to 33 \\___\\___ line lengths. （阿施要求他的受试者33 \\___\\___线条长度。）\n答案：match\n解析：定位第4段末尾。原文：\"asked them to match this in terms of length\"（要求他们将其长度……相匹配）。",
          "questionId": "q7"
        },
        {
          "questionNumber": 34,
          "text": "(4) 题目 34：To test the extent to which people would conform, he placed his subjects with colleagues who gave 34 \\___\\___ responses. （为了测试人们从众的程度，他将受试者与给出34 \\___\\___回答的同事安排在一起。）\n答案：untrue / wrong\n解析：定位第5段开头。原文：\"The stooges gave untrue responses\"（托儿给出了不真实的回答）。或者第5段中部的\"wrong answer\"。",
          "questionId": "q8"
        },
        {
          "questionNumber": 35,
          "text": "(5) 题目 35：He found that his subjects agreed with his colleagues 32% of the time, although they admitted to feeling 35 \\___\\___ about giving their answer. （他发现受试者有32%的时间同意同事的观点，尽管他们承认对自己给出的答案感到35 \\___\\___。）\n答案：anxious\n解析：定位第5段中部。原文：\"they also stated that when they did give wrong answers it made them anxious\"（他们也表示，当他们给出错误答案时，这让他们感到焦虑）。",
          "questionId": "q9"
        }
      ],
      "questionRange": {
        "start": 31,
        "end": 35
      },
      "text": "答案：beans\n解析：定位第3段。原文：\"estimate the number of beans in a bottle\"（估计瓶子里豆子的数量）。\n答案：group\n解析：定位第3段末尾。原文：\"discovered that they had stayed with the group answer\"（发现他们坚持了小组的答案）。\n答案：match\n解析：定位第4段末尾。原文：\"asked them to match this in terms of length\"（要求他们将其长度……相匹配）。\n答案：untrue / wrong\n解析：定位第5段开头。原文：\"The stooges gave untrue responses\"（托儿给出了不真实的回答）。或者第5段中部的\"wrong answer\"。\n答案：anxious\n解析：定位第5段中部。原文：\"they also stated that when they did give wrong answers it made them anxious\"（他们也表示，当他们给出错误答案时，这让他们感到焦虑）。"
    },
    {
      "sectionTitle": "3. 笔记填空（Questions 36–40）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 36,
          "text": "(1) 题目 36：Compliance... illustrated by the results of the research conducted by 36 \\___\\___ （顺从……由36 \\___\\___进行的研究结果所说明）\n答案：Asch\n解析：定位第6段。原文：\"If we look back to the Asch study, we can see that the subjects were simply complying\"（如果我们回顾阿施的研究，我们可以看到受试者只是顺从）。",
          "questionId": "q10"
        },
        {
          "questionNumber": 37,
          "text": "(2) 题目 37：37 \\___\\___ （第二种过程名称）\n答案：Internalization\n解析：定位第6段中部。原文：\"In a process known as internalization...\"（在一个被称为内化的过程中……）。这是Kelman提出的第二个过程。",
          "questionId": "q11"
        },
        {
          "questionNumber": 38,
          "text": "(3) 题目 38：majority view is considered most 38 \\___\\___ view （多数人的观点被认为是最38 \\___\\___的观点）\n答案：valid\n解析：定位第6段中部。原文：\"subjects do actually see the view of the group as the more valid one\"（受试者确实将群体的观点视为更有效的观点）。",
          "questionId": "q12"
        },
        {
          "questionNumber": 39,
          "text": "(4) 题目 39：39 \\___\\___ （第三种过程名称）\n答案：Identification\n解析：定位第6段后半部分。原文：\"Kelman calls this identification\"（凯尔曼称此为认同）。这是第三个过程。",
          "questionId": "q13"
        },
        {
          "questionNumber": 40,
          "text": "(5) 题目 40：typical of 40 \\___\\___ （是40 \\___\\___的典型特征）\n答案：teenagers\n解析：定位第6段末尾。原文：\"It frequently happens with teenagers...\"（这经常发生在青少年身上……）。",
          "questionId": "q14"
        }
      ],
      "questionRange": {
        "start": 36,
        "end": 40
      },
      "text": "答案：Asch\n解析：定位第6段。原文：\"If we look back to the Asch study, we can see that the subjects were simply complying\"（如果我们回顾阿施的研究，我们可以看到受试者只是顺从）。\n答案：Internalization\n解析：定位第6段中部。原文：\"In a process known as internalization...\"（在一个被称为内化的过程中……）。这是Kelman提出的第二个过程。\n答案：valid\n解析：定位第6段中部。原文：\"subjects do actually see the view of the group as the more valid one\"（受试者确实将群体的观点视为更有效的观点）。\n答案：Identification\n解析：定位第6段后半部分。原文：\"Kelman calls this identification\"（凯尔曼称此为认同）。这是第三个过程。\n答案：teenagers\n解析：定位第6段末尾。原文：\"It frequently happens with teenagers...\"（这经常发生在青少年身上……）。"
    }
  ]
}
  );
})(typeof window !== "undefined" ? window : globalThis);
